...
const BookEditCount = () => {
   ...
   const handleSubmit = async (e) => {
      ...
      try {
         ...
         const res = await axios.patch(`book_variant/${stock_no}/`, bookcount,
            {
               headers: {
   ...
   useEffect(() => {
      ...
      const fetchVariant = async () => {
         try {
            const access_token = JSON.parse(localStorage.getItem('access'));
            const res = await axios.get(`book_variant/${stock_no}/`,
               {
                  headers: {
                     Authorization: `Bearer ${access_token}`
                  }
            ...
            console.log(res)
            setBookCount({
               'book': res.data.book.id,
               'language': res.data.language.id,
               'publisher': res.data.publisher.id,
               'publishing_year': res.data.publishing_year,
               'isbn': res.data.isbn,
               'stock_no': res.data.stock_no,
               'price': res.data.price,
            })
            setSelect({
               'publisher_name': res.data.publisher.publisher_name,
               'language_name': res.data.language.language,
            })
         } catch (error) {
            console.log(error.response.data)
         }
                     ...
                     <div >
                        <div >
                           <select className='form-select' name="publisher" onChange={handleOnChange}>
                              <option value={bookcount.publisher}>{select.publisher_name}</option>
                              {publisher.map((pub) => {
                                 return (
               ...
               <div className={style.category}>
                  ...
                  <div >
                     <select className='form-select' name="language" onChange={handleOnChange}>
                        <option value={bookcount.language}>{select.language_name}</option>
                        {language.map((lang) => {
                           return (
